...
NARDOVE.Main = (function () {
		...
		function init() {
				$.ajax({
						url: "http://localhost:8000/can/can.svg",
						dataType: "xml",
						success: setup,
						error: function () {
		...
		function setup(svgData) {
				...
				$(svgData).find("#Layer_1")
						...
						.each(function (i) {
								...
								if (fill === "#FF0000") {
										movingParticles.push(new NARDOVE.Particle(ctx, bounds, point.x,
												point.y));
								}
		...
		function renderTriangles(inColor) {
				...
				(triangleCount < MaxTriangles) ? triangleCount++ : hasReachMaxTriangles =
						true;
		}
		...
		function renderCAN() {
				if (!stopCALettersAnimation) {
						...
						if (caLinePosX.toFixed(3) == targetPosX && caLinePosY.toFixed(3) ==
								targetPosY) {
				...
				ctx.lineJoin = "round";
				ctx.lineWidth = 3.5;
				ctx.strokeStyle = (colors[colorIndex].length > 4) ? colors[colorIndex][4] :
						"#FFFFFF";

				// Draw moving point
				if (!stopCALettersAnimation) {
						ctx.fillStyle = (colors[colorIndex].length > 4) ? colors[colorIndex][4] :
								"#FFFFFF";
						ctx.beginPath();
