...
function setup() {
  ...
  img = loadImage(fileName, function () {
    img.loadPixels();
    //console.log(pixels)
    for (var x = 0; x < img.width; x += resolution) {
      for (var y = 0; y < img.height; y += resolution) {
        ...
        if (b < 200) {
          vehicles.push(new Vehicle(new p5.Vector(random(width), random(
            height)), 8, 0.9, new p5.Vector(x, y)));
        }
      }
    }
  });
...
Vehicle.prototype = {
  ...
  display: function () {
    ...
    fill(Math.floor((dis * 20 + 50) * 360 / 250), Math.floor((dis * 4) * 360 /
      250), Math.floor((360 - dis) * 360 / 250));
