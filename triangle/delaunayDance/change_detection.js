(function (exports) {
		...
		DetectPoints.prototype.draw = function() {
				...
				if (WEBCAM.localMediaStream) {
						...
						for (var ii = 0; i < this.width; ii += this.sampleRate) {
							for (var jj = 0; jj < this.height; jj += this.sampleRate) {
								var data = this.ctx.getImageData(ii, jj, 1, 1).data;
								this.currentColorY[index] = data;
								var colorDifference = colorDiff(this.previousColorY[index], this.currentColorY[
										index]);
								if (colorDifference > this.colorDiffShrehold) {
										//this.points.push(new Point(i, j));
								...
								this.previousColorY[index] = this.currentColorY[index];
								index++;
				...
				for(var itt = 1; itt< rawPointsY.length - 1 ; itt++){
						if(rawPointsY[itt][0] + this.sampleRate !== rawPointsY[itt+1][0] || 
							rawPointsY[itt][0] - this.sampleRate !== rawPointsY[itt-1][0]){
							//getIndexes.push(it);
							// this.points.push(new Point(rawPoints[it][0], rawPoints[it][1],"#ff00ff"));
				...
				for(var x=0; x<afterX.length; x++){
					for(var y = 0; y<afterY.length; y++){
						if(afterX[x] === afterY[y]){
							this.points.push(new Point(afterX[x][0], afterX[x][1],"#ff00ff"));
						}
					}
				}
