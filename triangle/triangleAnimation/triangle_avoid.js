...
(function(exports){
    ...
    for(var i = -100; i < 1400; i += 200){
      for( var k = -100; k < 400; k += 200){
        ...
        this.vehicles.push(new Vehicle(new PVector(i, k), 8, 0.9, new PVector(i, k), this.canvas, "static"));
        index++;
      }
    }
    ...
    for (var j = 22; j < this.vertices.length; j++) {
      ...
      this.vertices[j] = [x, y];
      this.vehicles.push(new Vehicle(new PVector(x, y), 8, 0.9, new PVector(x, y), this.canvas, "dynamic"));
    }
    ...
    for (var i = this.triangles.length; i >= 3; i -= 3) {
      ...
      // Math.floor(250 - k * 2)+ ')';
      // }else{
      this.context.fillStyle = 'rgb('+ Math.floor(k*1.4)+ ','+ Math.floor(250 - k * 4) + ',' +
      Math.floor(250 - k * 2)+ ')';
    //}
