  ...
  function DelaunayAnimation (can){
    ...
    DelaunayAnimation.prototype.draw = function(){
      ...
      for(var j=0; j<this.vertices.length; j++){
        ...
        this.vertices[j][0] = this.vertices[j][0] += ( Math.random() - 0.5 );
        this.vertices[j][1] = this.vertices[j][1] += ( Math.random() - 0.5 );
      }
      ...
      for(i = this.triangles.length; i>=3; i-=3 ) {
        ...
        this.context.moveTo(this.vertices[this.triangles[i-1]][0], this.vertices[this.triangles[i-1]][1]);
        this.context.lineTo(this.vertices[this.triangles[i-2]][0], this.vertices[this.triangles[i-2]][1]);
        this.context.lineTo(this.vertices[this.triangles[i-3]][0], this.vertices[this.triangles[i-3]][1]);
